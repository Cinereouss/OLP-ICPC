...
void bacba(float a, float b, float c, float d)
{
  ...
  dt=pow(b,2)-3*a*c;
  k=(9*a*b*c-2*pow(b,3)-27*pow(a,2)*d)/(2*sqrt(pow(abs(dt),3)));
  if(dt>0)
  {
    if(abs(k)<=1)
    {
      ...
      x2=(2*sqrt(dt)*cos(acos(k)/3-(2*PI/3))-b)/(3*a);
		cout << (int)min(min(x1, x2), x3);
    }
    if(abs(k)>1)
    {
      x=((sqrt(dt)*abs(k))/(3*a*k))*(pow((abs(k)+sqrt(pow(k,2)-1)),1.0/3)+pow((abs(k)-sqrt(pow(k,2)-1)),1.0/3))-(b/(3*a));
      cout << (int)x;
    }
   }
  else if(dt==0)
